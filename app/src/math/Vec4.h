...
public:
    ...
    static arma::mat convertToArmaMatrix(const std::vector<Vec4<T>>& points) {
        arma::mat armaPoints(3, points.size());
        for (size_t i = 0; i < points.size(); ++i) {
            armaPoints(0, i) = points[i].x;
            armaPoints(1, i) = points[i].y;
            armaPoints(2, i) = points[i].z;
        }
        return armaPoints;
    }

    static bool calculatePlane(const std::vector<Vec4<T>>& points, Vec4<T>& supportVector, Vec4<T>& normalVector) {
        if(points.size() <= 3)
            return false;
        ...
        // Convert Vec4f points to Armadillo matrix
        arma::mat armaPoints = convertToArmaMatrix(points);

        // 1. Compute the centroid as the support vector
        arma::vec centroid = arma::mean(armaPoints, 1);

        // Set the support vector
        supportVector = Vec4f(centroid(0), centroid(1), centroid(2));

        // 2. Center the points around the centroid
        arma::mat centeredPoints = armaPoints.each_col() - centroid;

        // 3. Compute the covariance matrix
        arma::mat covMatrix = centeredPoints * centeredPoints.t() / (points.size() - 1);

        // 4. Perform eigen decomposition
        arma::vec eigenvalues;
        arma::mat eigenvectors;
        arma::eig_sym(eigenvalues, eigenvectors, covMatrix);

        // 5. The eigenvector corresponding to the smallest eigenvalue is the normal vector
        arma::vec normal = eigenvectors.col(0);

        // Set the normal vector
        normalVector = Vec4<T>(normal(0), normal(1), normal(2), 0);
    }

    static Vec4<T> intersectRayWithPlane(const Vec4<T>& planeP, const Vec4<T>& planeN,
                                const Vec4<T>& rayP, const Vec4<T>& rayDir) {
        // Calculate the denominator
        float denom = planeN.dot(rayDir);
        ...
        float t = planeN.dot(planeP - rayP) / denom;

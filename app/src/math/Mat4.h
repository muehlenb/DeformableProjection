...
public:
     ...
     * new vector.
     *
     * Like normal multiplication but with the assumption that the bottom row is (0, 0, 0, 1).
     */
    Vec4<T> fastMultiply(const Vec4<T> v) const {
        T dest[4];
     ...
     * @param verticalFoV       The opening angle of the 'eye' or camera in *degree* (y axis).
     ...
     */
    static Mat4<T> perspectiveTransformation(T aspectRatio = 1.f, T verticalFoV = 75.f, T near = 0.01f, T far = 1000.f){
        Mat4<T> result;

        verticalFoV *= (M_PI / 180);

        result.data[0] = cosf(verticalFoV / 2) / sinf(verticalFoV / 2) / aspectRatio;
        result.data[5] = cosf(verticalFoV / 2) / sinf(verticalFoV / 2);
    ...
    static Mat4<T> customPerspectiveTransformation(T horizontalFoV = 75.f, T verticalFoV = 65.f, T horizontalOffset = 0.f, T verticalOffset = 0.f, T near = 0.01f, T far = 1000.f){
        Mat4<T> result;

        // Transform fov from degree to radians:
        T verticalFoV_rad = verticalFoV * (M_PI / 180);
        T horizontalFoV_rad = horizontalFoV * (M_PI / 180);

        // Calculate the scale factor for the x- and y-axes:
        T scaleY = 1 / tan(verticalFoV_rad / 2);    // cotangent of half the verticalFoV
        T scaleX = 1 / tan(horizontalFoV_rad / 2);  // cotangent f half the horizontalFoV

        // Set the values in the result matrix:
        result.data[0] = scaleX; // Scale on the x-axis
        result.data[5] = scaleY; // Scale on the y-axis
        result.data[10] = (far + near) / (far - near); // Scale und translation on the z-axis
        result.data[11] = 1; // perspective division
        result.data[14] = (-2 * far * near) / (far - near); // Term for z-Clipping
        result.data[15] = 0; // For the perspective

        result.data[12] = horizontalOffset;
        result.data[13] = verticalOffset;
     ...
     */
    static Mat4<T> getRotationMatrix(Vec4<T> start, Vec4<T> end, Vec4<T> up = Vec4<T>(0,1,0,0), bool autoUpAxis = true, T dotEps = 0.9999){
        if(start.length() < 0.01f || end.length() < 0.01f)
            return Mat4<T>();

        // Search for a sufficiently perpendicular axis:
        Vec4<T> perpendicular = up;

        if(autoUpAxis){
            if(abs(start.dot(perpendicular)) > 0.9 || abs(end.dot(perpendicular)) > 0.9){
                perpendicular = Vec4<T>(1,0,0,0);
            }

            if(abs(start.dot(perpendicular)) > 0.9 || abs(end.dot(perpendicular)) > 0.9)
                perpendicular = Vec4<T>(0,0,1,0);
        }

        Mat4<T> rotStart;
        if(abs(start.dot(perpendicular)) < dotEps){
        ...
        Mat4<T> rotEnd;
        if(abs(end.dot(perpendicular)) < dotEps){
            rotEnd.data[0] = end.x;
            rotEnd.data[1] = end.y;
            rotEnd.data[2] = end.z;
...
/*
    static Mat4 fromEigen(Eigen::Matrix4f mat){
        float tmp[16] = {
    ...
    static Eigen::Matrix4f toEigen(Mat4 mat){
        Eigen::Matrix4f tmp;
        tmp(0,0) = mat.data[0];
        tmp(1,0) = mat.data[1];
        tmp(2,0) = mat.data[2];
